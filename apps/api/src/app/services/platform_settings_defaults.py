...
PLATFORM_REQUIRED_OPERATION_RETRY_INSTRUCTION = (
    "Необходимо вызвать хотя бы один инструмент перед ответом. "
    "Не отвечай по памяти — используй результаты вызовов. "
    "Выбери наиболее подходящий инструмент и верни tool_call."
)

PLATFORM_OPERATION_RULES_TEXT = (
    "ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА — соблюдай без исключений:\n"
    "1. Если пользователь запрашивает РЕАЛЬНЫЕ ДАННЫЕ (записи, значения, количества, статусы, конфигурацию) — сначала вызови инструмент. Не отвечай по памяти.\n"
    "2. ИСКЛЮЧЕНИЕ: если вопрос только о том, какие источники данных или инструменты доступны (мета-вопрос о возможностях) — можно ответить напрямую из capability card.\n"
    "3. Финальный ответ на вопросы о данных — только после получения результатов вызовов.\n"
    "4. Не придумывай значения — используй только данные из результатов вызовов."
)

PLATFORM_INTENT_MESSAGES: Dict[str, str] = {
    "agent_start": "Запускаю выполнение агента",
    "final_answer": "Формирую финальный ответ",
    "tool_call": "Вызываю инструмент: {tool_name}",
}
