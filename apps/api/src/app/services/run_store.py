...
Provides async methods to create runs, add steps, and finalize runs.
"""
import uuid
import time
...
class RunStore:
    """
    Manages agent run logging.
    Usage:
        store = RunStore(session)
        run_id = await store.start_run(...)
        await store.add_step(run_id, "tool_call", {...})
    ...
    async def finish_run(
        ...
        """Finalize a run with status and metrics."""
        ...
        run.total_tool_calls = tool_calls_result.scalar() or 0
        
        # Aggregate tokens if provided
