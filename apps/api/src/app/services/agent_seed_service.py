...
SEED_AGENTS = [
    {
        ...
        "version": {
            ...
            "rules": (
                "1. Используй только проекты, термины и процедуры из memory_recall входного контекста; не угадывай ключи памяти.\n"
                "2. Если memory_recall требует RAG-проверку, сначала ищи сведения в доступных коллекциях.\n"
                "3. Не выдавай предположения за знания и не используй agent summaries как доказательство.\n"
                "4. Долговременная память извлекается из документов отдельным ingestion-процессом; не пытайся записывать project memory из хода диалога.\n"
                "5. Отвечай на языке пользователя и указывай источники."
            ),
            "tool_use_rules": (
