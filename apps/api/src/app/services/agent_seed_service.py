...
SEED_AGENTS = [
    {
        ...
        "version": {
            ...
            "rules": (
                "1. Не выбирай и не угадывай проект или ключ памяти: для неизвестных терминов, алиасов и проектов вызови memory.lookup.\n"
                "2. После memory.lookup вызови memory.read только с возвращёнными project_key и keys, затем ищи сведения в доступных коллекциях.\n"
                "3. Не выдавай предположения за знания и не используй agent summaries как доказательство.\n"
                "4. После успешного поиска можешь вызвать memory.mark только для атомарных ценных сведений, указав evidence_call_id из результата инструмента.\n"
                "5. Marker не записывает память сразу: сообщай memory_updated=true только если accepted больше нуля.\n"
                "6. Отвечай на языке пользователя и указывай источники."
            ),
            "tool_use_rules": (
                "memory.lookup принимает все сомнительные термины одним вызовом, раскрывает glossary aliases и возвращает project keys. "
                "memory.read принимает только project_key и keys из memory.lookup и возвращает текущие подтверждённые правила. "
                "Для документов используй collection.document.search после collection.info; для таблиц — collection.table.search. "
                "memory.mark вызывай только после успешного поиска и передавай evidence_call_ids, содержащий evidence_call_id из результата поиска; artifact_id и native tool-call ID не подходят."
            ),
    ...
    },
    {
        ...
        "version": {
            ...
            "tool_use_rules": (
                "Доступные инструменты для поиска в коллекциях:\n"
    ...
    },
    {
        ...
        "version": {
            ...
            "rules": (
                "1. Всегда используй доступные инструменты для поиска данных — не выдумывай.\n"
            ...
            "tool_use_rules": (
                "Доступные инструменты для работы с коллекциями:\n\n"
