...
def test_jwt_expiration():
    ...
    expired_token = encode_jwt(expired_payload, ttl_seconds=-3600)
    
    with pytest.raises(Exception):  # HTTPException from decode_jwt
...
def test_get_bearer_token():
    ...
    # Missing token - should raise HTTPException
    with pytest.raises(Exception):  # HTTPException
...
def test_validate_password_strength():
    ...
    # Strong password
    is_valid, _ = validate_password_strength("StrongPassword123!")
    assert is_valid is True
    
    # Weak passwords
    is_valid, _ = validate_password_strength("123")
    assert is_valid is False  # Too short
    
    is_valid, _ = validate_password_strength("password")
    assert is_valid is False  # No numbers/special chars
    
    is_valid, _ = validate_password_strength("PASSWORD123")
    assert is_valid is False  # No lowercase
    
    is_valid, _ = validate_password_strength("password123")
    assert is_valid is False  # No uppercase/special chars
