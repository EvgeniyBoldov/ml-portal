...
class ChatsRepository(BaseRepository[Chats]):
    ...
    def get_chat_with_messages(self, chat_id: str) -> Optional[Chats]:
...
class ChatMessagesRepository(BaseRepository[ChatMessages]):
    """Repository for chat messages"""
    
    def __init__(self, session: Session):
        super().__init__(session, ChatMessages)
    ...
    def search_messages(self, chat_id: str, query: str, limit: int = 50) -> List[ChatMessages]:
        ...
        stmt = select(ChatMessages).where(
            and_(
                ...
                ChatMessages.content.astext.ilike(f"%{query}%")
            )
...
# Async versions
class AsyncChatsRepository(AsyncBaseRepository[Chats]):
...
class AsyncChatMessagesRepository(AsyncBaseRepository[ChatMessages]):
