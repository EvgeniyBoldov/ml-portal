"""
Enhanced async repository with production-grade features:
- Tenant isolation with scope filtering (local + global)
- Optimistic locking with version field
- Cursor-based pagination
- Error mapping
- Bulk operations
- Only async operations (no sync Session.query)
"""
...
class AsyncRepository(Generic[T]):
    ...
    async def create(self, **kwargs) -> T:
        ...
        try:
            ...
            logger.debug(f"Created {self.model.__name__}: {getattr(instance, 'id', 'no-id')}")
            return instance
...
class AsyncTenantRepository(Generic[T]):
    """Async base repository with mandatory tenant isolation and scope filtering"""
    ...
    def _build_scope_filter(self, stmt):
        """Build scope filter for search: (tenant_id==current AND scope='local') OR (scope='global')"""
        if hasattr(self.model, 'scope'):
            # For models with scope field (like RAGDocument)
            scope_filter = or_(
                and_(
                    self.model.tenant_id == self.tenant_id,
                    self.model.scope == 'local'
                ),
                self.model.scope == 'global'
            )
            stmt = stmt.where(scope_filter)
        else:
            # For models without scope field, use tenant isolation only
    ...
    def _encode_cursor(self, entity: T) -> str:
        ...
        cursor_data = {
            'created_at': entity.created_at.isoformat() if hasattr(entity, 'created_at') else datetime.utcnow().isoformat(),
            'id': str(entity.id)
        }
    ...
    def _decode_cursor(self, cursor: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'created_at' in cursor_data:
                filters['created_at'] = {'lt': datetime.fromisoformat(cursor_data['created_at'])}
            if 'id' in cursor_data:
                filters['id'] = {'lt': uuid.UUID(cursor_data['id'])}
            return filters
        except Exception:
    ...
    async def create(self, tenant_id: uuid.UUID, **kwargs) -> T:
        ...
        try:
            logger.info(
                "Creating %s with tenant_id=%s and data=%s",
                self.model.__name__,
                tenant_id,
                kwargs,
            )
            ...
            logger.debug(
                "Added %s instance to session (temp id=%s)",
                self.model.__name__,
                getattr(instance, 'id', 'not-set'),
            )

            await self.session.flush()

            logger.debug(
                "Flushed %s instance (id=%s)",
                self.model.__name__,
                getattr(instance, 'id', 'not-set'),
            )
            ...
            logger.info(
                "Created %s: %s",
                self.model.__name__,
                getattr(instance, 'id', 'no-id'),
            )
            return instance
        except IntegrityError as e:
            logger.error(
                "Integrity error creating %s: %s",
                self.model.__name__,
                e,
                exc_info=True,
            )
    ...
    async def list(self, tenant_id: uuid.UUID, filters: Optional[Dict[str, Any]] = None,
                   ...
                   cursor: Optional[str] = None, use_scope_filter: bool = False) -> Tuple[List[T], Optional[str]]:
        """List records with cursor-based pagination and optional scope filtering"""
        try:
            stmt = select(self.model)
            
            if use_scope_filter:
                # Use scope filter for search operations
                stmt = self._build_scope_filter(stmt)
            else:
                # Use tenant isolation for regular list operations
            ...
            if cursor:
                ...
                stmt = self._apply_filters(stmt, cursor_filters)
    ...
    async def count(self, tenant_id: uuid.UUID, filters: Optional[Dict[str, Any]] = None, use_scope_filter: bool = False) -> int:
        """Count records with tenant isolation and optional scope filtering"""
        try:
            ...
            if use_scope_filter:
                ...
                # Use tenant isolation for regular count operations
                filters = self._ensure_tenant_filter(filters or {})
