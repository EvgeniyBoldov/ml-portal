...
from sqlalchemy import select, insert, func, or_, tuple_
...
class AsyncUsersRepository:
    ...
    async def list_users(
        ...
        if query:
            ...
            filters.append(
                or_(
                    func.lower(Users.login).like(pattern),
                    func.lower(Users.email).like(pattern),
                )
            )
        ...
        if cursor_id:
            if cursor_created_at:
                data_stmt = data_stmt.where(
                    tuple_(Users.created_at, Users.id)
                    > tuple_(cursor_created_at, cursor_id)
                )
            else:
                data_stmt = data_stmt.where(Users.id > cursor_id)

        data_stmt = data_stmt.order_by(Users.created_at, Users.id).limit(limit + 1)
        ...
        return users, next_cursor, has_more, total
