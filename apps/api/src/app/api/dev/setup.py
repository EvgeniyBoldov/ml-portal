...
from app.schemas.users import UserCreateRequest as UserCreate, UserResponse
import logging
...
async def create_superuser(
    ...
    existing_admin = repo.get_user_by_login(login)
    if existing_admin:
...
async def setup_status(session: Session = Depends(get_session)):
    ...
    # Count admin users
    admin_count = repo.count_users_by_role("admin")
