...
async def build_collection_response(service: CollectionService, collection) -> CollectionResponse:
    ...
    if collection.current_version:
        current_version_payload = CollectionVersionResponse(
            ...
            usage_rules=collection.current_version.usage_rules,
    ...
    return CollectionResponse(
        ...
        lifecycle_status=getattr(collection, "lifecycle_status", "active"),
        deprecated_at=collection.deprecated_at.isoformat() if getattr(collection, "deprecated_at", None) else None,
        retention_days=getattr(collection, "retention_days", 14),
