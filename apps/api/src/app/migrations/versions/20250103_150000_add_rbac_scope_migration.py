...
def upgrade():
    ...
    bind = op.get_bind()
    scope_enum = postgresql.ENUM('local', 'global', name='documentscope', create_type=True)
    scope_enum.create(bind, checkfirst=True)
    
    # Create DocumentStatus enum  
    status_enum = postgresql.ENUM('uploading', 'processing', 'processed', 'failed', 'archived', name='documentstatus', create_type=True)
    ...
    existing_cols = {c['name'] for c in inspector.get_columns('ragdocuments')}
    if 'scope' not in existing_cols:
        op.add_column('ragdocuments', sa.Column('scope', scope_enum, nullable=False, server_default='local'))
    
    # Add tenant_id column (nullable for global docs)
    if 'tenant_id' not in existing_cols:
        op.add_column('ragdocuments', sa.Column('tenant_id', postgresql.UUID(as_uuid=True), nullable=True))
    
    # Add versioning fields for global documents
    if 'global_version' not in existing_cols:
        op.add_column('ragdocuments', sa.Column('global_version', sa.Integer(), nullable=True))
    ...
    # First, drop the existing column and recreate it
    # Update status column to use new enum type. If already enum, skip recreation.
    status_col = next((c for c in inspector.get_columns('ragdocuments') if c['name'] == 'status'), None)
    if status_col is not None and not isinstance(status_col['type'], postgresql.ENUM):
        op.drop_column('ragdocuments', 'status')
        op.add_column('ragdocuments', sa.Column('status', status_enum, nullable=False, server_default='processed'))
    
    # =============================================================================
    # UPDATE RAG_CHUNKS TABLE
    # =============================================================================
    
    # Check if rag_chunks table exists before adding columns
    ...
    # Check if audit_logs table exists before adding columns
    result = connection.execute(sa.text("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables 
            WHERE table_schema = 'public' 
    ...
    # RAG Chunks indexes (only if table exists)
    if table_exists:
...
def downgrade():
    ...
    # Check if rag_chunks table exists before dropping index
    connection = op.get_bind()
    result = connection.execute(sa.text("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables 
    ...
    # Check if audit_logs table exists before dropping indexes
    result = connection.execute(sa.text("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables 
            WHERE table_schema = 'public' 
    ...
    # Drop RAG Chunks columns (only if table exists)
    if table_exists:
    ...
    # Drop Audit logs columns (only if table exists)
    if audit_logs_exists:
