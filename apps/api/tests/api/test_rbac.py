...
@pytest.fixture
def admin_user(db_session_commit: Session):
    """Create admin user for testing."""
    repo = UsersRepository(db_session_commit)
    ...
    db_session_commit.commit()  # Commit to make user available for authentication
    return user


@pytest.fixture
def editor_user(db_session_commit: Session):
...
@pytest.fixture
def reader_user(db_session_commit: Session):
...
# Global variable to track current test user role
_current_test_role = "reader"

def get_auth_headers(client: TestClient, login: str, password: str):
    """Get authorization headers for a user."""
    global _current_test_role
    # Set the test user role globally
    _current_test_role = login
    # Since we're mocking authentication, just return a dummy token
...
class TestRBAC:
    ...
    def test_inactive_user_cannot_access_anything(self, client: TestClient, db_session: Session):
...
class TestAdminAPI:
    ...
    def test_reset_user_password(self, client: TestClient, admin_user, editor_user):
        ...
        password_data = {"new_password": "NewEditorPassword123!"}
        response = client.post(f"/api/admin/users/{editor_user.id}/password", json=password_data, headers=headers)
        assert response.status_code == 200
    ...
    def test_create_pat_token(self, client: TestClient, admin_user, editor_user):
        ...
        assert data["scopes"] == ["api:read", "api:write"]
