...
import ChatTags from '@app/components/ChatTags';
...
export default function ChatSidebar() {
  ...
  const chats = useMemo(
    () => state.chatsOrder.map(id => state.chatsById[id]),
    [state.chatsOrder, state.chatsById]
  );
          ...
          <>
            ...
            <Button
              onClick={async () => {
                ...
                await renameChat(
                  renameTarget.id,
                  renameTarget.name || 'Без названия'
                );
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const tid = setTimeout(async () => {
                  try {
                    await deleteChatApiOnly(chat?.id || '');
                  } catch (e) {
                    console.error(e);
                  }
                  setPendingUndo(null);
      ...
      {pendingUndo && (
        <div className={styles.undoBar} role="status">
          <div className={styles.undoText}>
            Чат «{pendingUndo.name}» удалён. Отменить можно в течение{' '}
            {pendingUndo.secs} с.
          </div>
          <button
            ...
            onClick={() => {
              ...
              restoreChatLocal({
                id: pendingUndo.id,
                name: pendingUndo.name || null,
                tags: pendingUndo.tags || [],
              } as any);
              setPendingUndo(null);
            }}
          >
