...
export async function uploadFile(file: File, name?: string, tags?: string[]) {
  ...
  return apiRequest<{ id: string; status: string }>('/rag/upload', {
    method: 'POST',
    body: fd,
  });
}
...
export async function deleteRagDocument(doc_id: string) {
  return apiRequest<{ id: string; deleted: boolean }>(`/rag/${doc_id}`, {
    method: 'DELETE',
  });
}
...
export async function reindexRagDocument(doc_id: string) {
  return apiRequest<{ id: string; status: string }>(`/rag/${doc_id}/reindex`, {
    method: 'POST',
  });
}

export async function reindexAllRagDocuments() {
  return apiRequest<{ reindexed_count: number; total_documents: number }>(
    '/rag/reindex',
    { method: 'POST' }
  );
}
