/**
 * ModelsTable - Таблица моделей для переиспользования
 * 
 * Вынесено из ModelsPage для использования в PlatformSettingsPage
 */
...
import { DataTable, type DataTableColumn, Badge, Alert, ActionsButton, type ActionItem } from '@/shared/ui';
...
interface ModelsTableProps {
  /** Показывать только активные модели */
  activeOnly?: boolean;
  /** Скрывать заголовок и экшены */
  minimal?: boolean;
}

export function ModelsTable({ activeOnly = false, minimal = false }: ModelsTableProps) {
  ...
  const queryParams = useMemo(
    () => ({
      ...
      status: activeOnly ? 'available' : (filters.models_status as string) || undefined,
      search: debouncedQ || undefined,
      page: 1,
      size: 50,
    }),
    [debouncedQ, filters.models_type, filters.models_status, activeOnly]
  );
  ...
  // Если minimal mode, возвращаем только таблицу
  if (minimal) {
    return (
      <>
        {error && (
  ...
  // Полный режим с заголовком и экшенами
  return (
    <>
      {error && (
        <div style={{ padding: '16px', background: 'var(--danger-bg)', borderRadius: '8px', marginBottom: '16px' }}>
