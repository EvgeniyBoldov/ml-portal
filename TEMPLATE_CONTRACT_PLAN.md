...
плейсхолдерами. Агентский флоу: **discover → contract → describe → fill**.

`template_schema` (далее — **контракт**) — первичный артефакт. Из него:
- выводится `fill_input_schema` (JSON-форма, которую использует LLM при заполнении);
- валидируются входные `values`;
- строится семантическое `description` на следующем этапе.
...
- **Repeat marker as syntax.** `[]` у узла означает повторяемый блок; `()`
  содержит метаданные и ограничения; `.` задает вложенность.
- **Два режима таблиц (Both).** Приоритет — explicit repeat region (`[]` и
  marker-anchors); fallback — structural (без разметки, таблицы детектируются по
  структуре/шапке).
...
### Конвенции
- Скаляр: токен `{{user.name(name='ФИО')}}`.
- Табличный блок: `{{connections(name='Связности')[] ... }}`.
- Вложенность задается точкой: `source.ip`, `destination.name`.
- `[]` означает повторяемый блок, `()` содержит `name`, `description`, `type`,
  `required`, `min`, `max` и иные метаданные.
- `orientation`: `vertical` — рост вниз; `horizontal` — рост вправо.
- В Excel шаблон заполняется только по существующим строкам и столбцам: движок
  клонирует уже размеченную строку или колонку, но не создает новую структуру.
...
- Детерминированный parser-fallback оставляем только для вывода контракта, но не
...
- Описание строится только после схемы и использует контракт как источник истины.
