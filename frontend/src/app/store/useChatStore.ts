  ...
  devtools(
    persist(
      immer((set, _get) => ({
        ...
        addMessage: (chatId, message) =>
          set(state => {
            if (state.chats[chatId]) {
              ...
              state.chats[chatId].last_message_at =
        ...
        setMessages: (chatId, messages, hasMore, nextCursor) =>
          set(state => {
            if (state.chats[chatId]) {
        ...
        appendMessages: (chatId, messages, hasMore, nextCursor) =>
          set(state => {
            if (state.chats[chatId]) {
...
export const useChatMessages = (chatId: string) =>
  useChatStore(state => state.chats[chatId]?.messages || []);
...
export const useChatStats = () =>
  useChatStore(state => {
    ...
    const userMessages = chats.reduce(
      (sum, chat) =>
        sum + chat.messages.filter(msg => msg.role === 'user').length,
      0
    );
    const assistantMessages = chats.reduce(
      (sum, chat) =>
        sum + chat.messages.filter(msg => msg.role === 'assistant').length,
      0
    );
