...
4. Tenant в этом проекте означает отдел/рабочую область одной компании,
   а не жесткую SaaS security boundary. Доступ к данным регулируется
   sharing/RBAC policy; данные могут быть общими между отделами, если это
   явно разрешено продуктовой политикой.
5. Scope-приоритеты:
...
1. Всегда учитывать effective access policy при доступе к scoped-данным.
   `tenant_id` сам по себе не является запретом на sharing между отделами.
...
4. Effective access policy и scope resolution соблюдены.
