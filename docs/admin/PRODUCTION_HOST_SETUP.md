...
Все три значения должны быть абсолютными путями. До первого deploy создайте
`/etc/ml-portal/nginx` и `/etc/ml-portal/tls`, положите в nginx-каталог оба
файла `prod.conf` и `files.conf`, а также убедитесь, что каталог моделей
существует. Controller проверяет эти пути до применения миграции.
