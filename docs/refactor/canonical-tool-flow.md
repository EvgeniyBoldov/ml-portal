...
- Separate raw discovered capabilities from published runtime operations.
...
5. `ToolResolver` maps a raw tool to a canonical published runtime operation.
...
- Collection-bound operations must declare supported `collection_types` in canonical semantics.
...
- Callable operations appendix shows exact invoke name, concise description, collection binding, and input schema.
- Internal-only fields such as raw handler aliases stay out of LLM prompt and belong in operator tooling only if needed.

## Maintenance
- Adding a new runtime operation requires:
